...
public class Application {
	...
	public static void main(String[] args) {
		...
		dao.insert("KARTIK", 230510500, new Date(), "CSE");
...
//		}
		...
		// update 1st record
		if (!data.isEmpty()) {
			Student s = data.get(0); // Corrected from getFirst()
		...
		// delete last row
		if (!data.isEmpty()) {
			// Refresh data list in case the size changed
			data = dao.find(); 
			Student s = data.get(data.size() - 1); // Corrected from getLast()
